...
func TestUnmarshalTCPFlags(t *testing.T) {
	...
	var flags TCPFlags
	if err := nibble.Unmarshal(data, &flags, nibble.BigEndian); err != nil {
...
func TestUnmarshalGamePacket(t *testing.T) {
	...
	var pkt GamePacket
	if err := nibble.Unmarshal(data, &pkt, nibble.LittleEndian); err != nil {
		t.Fatalf("Unmarshal: %v", err)
	}
...
func TestRoundTripTCPFlags(t *testing.T) {
	...
	data, err := nibble.Marshal(&original, nibble.BigEndian)
	if err != nil {
		t.Fatalf("Marshal: %v", err)
	}

	var decoded TCPFlags
	if err := nibble.Unmarshal(data, &decoded, nibble.BigEndian); err != nil {
		t.Fatalf("Unmarshal: %v", err)
	}
...
func TestRoundTripGamePacket(t *testing.T) {
	...
	data, err := nibble.Marshal(&original, nibble.LittleEndian)
	if err != nil {
		t.Fatalf("Marshal: %v", err)
	}
...
func TestExplain(t *testing.T) {
	...
	out, err := nibble.Explain(data, TCPFlags{}, nibble.BigEndian)
	if err != nil {
...
func TestExplainMultiByte(t *testing.T) {
	...
	out, err := nibble.Explain(data, GamePacket{}, nibble.LittleEndian)
	if err != nil {
		t.Fatalf("Explain: %v", err)
	}
...
func TestEndiannessProducesDifferentBytes(t *testing.T) {
	...
	le, err := nibble.Marshal(&pkt, nibble.LittleEndian)
	if err != nil {
		t.Fatalf("Marshal LE: %v", err)
	}
	be, err := nibble.Marshal(&pkt, nibble.BigEndian)
	if err != nil {
	...
	if err := nibble.Unmarshal(le, &leOut, nibble.LittleEndian); err != nil {
		t.Fatalf("Unmarshal LE: %v", err)
	}
	if err := nibble.Unmarshal(be, &beOut, nibble.BigEndian); err != nil {
...
func TestErrInsufficientData(t *testing.T) {
	err := nibble.Unmarshal([]byte{}, &TCPFlags{}, nibble.BigEndian)
	if !errors.Is(err, nibble.ErrInsufficientData) {
...
func TestErrBitWidthInvalid(t *testing.T) {
	...
	err := nibble.Unmarshal([]byte{0xFF, 0xFF}, &Bad{})
	if !errors.Is(err, nibble.ErrBitWidthInvalid) {
...
func TestErrUnsupportedType(t *testing.T) {
	...
	err := nibble.Unmarshal([]byte{0xFF}, &Bad{})
	if !errors.Is(err, nibble.ErrUnsupportedType) {
...
func TestDiffTypeMismatch(t *testing.T) {
	_, err := nibble.Diff(TCPFlags{}, GamePacket{})
	if err == nil {
...
func TestMarshalTCPSYNACK(t *testing.T) {
	...
	data, err := nibble.Marshal(&flags, nibble.BigEndian)
	if err != nil {
		t.Fatalf("Marshal: %v", err)
	}
	if len(data) != 1 {
